...
const donorNavigation = [
  { label: "Dashboard", href: "/donor/dashboard", icon: Home },
...
function DonorSidebar({ pathname, onNavigate }: { pathname: string; onNavigate?: () => void }) {
  return (
    <aside className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
      <div className="flex h-16 items-center gap-3 px-5">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Donor workspace</p>
        </div>
      </div>
      <Separator />
...
export function DonorLayout({ children }: DashboardLayoutProps) {
  ...
  const initials = useMemo(() => {
    const source = user?.username ?? user?.email ?? "Donor"
    return source.slice(0, 2).toUpperCase()
  }, [user])

  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.14),transparent_34%),linear-gradient(180deg,#fbfdf8,#f7f7f2)]">
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-72 lg:flex-col lg:border-r lg:bg-sidebar/95 lg:backdrop-blur">
        <DonorSidebar pathname={pathname} />
      </div>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="relative h-full w-80 max-w-[85vw] border-r bg-sidebar shadow-xl">
            ...
            <DonorSidebar pathname={pathname} onNavigate={() => setMobileOpen(false)} />
          </div>
        </div>
      ) : null}
        ...
        <header className="sticky top-0 z-30 border-b bg-background/85 backdrop-blur-xl">
          <div className="flex h-16 items-center gap-4 px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-3 rounded-full border bg-card py-1 pl-1 pr-3 shadow-sm">
              <div className="flex size-8 items-center justify-center rounded-full bg-emerald-700 text-xs font-semibold text-white">
                {initials}
              </div>
              <div className="hidden leading-tight sm:block">
                <p className="text-sm font-medium">{user?.username ?? "Donor"}</p>
